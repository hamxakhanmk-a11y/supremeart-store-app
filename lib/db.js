...
async function ensureTables() {
  ...
  // Case-insensitive unique SKU: BRG-001 and brg-001 are treated as the same
  await sql`DROP INDEX IF EXISTS parts_sku_unique`;
  await sql`CREATE UNIQUE INDEX IF NOT EXISTS parts_sku_unique_ci ON parts(LOWER(sku)) WHERE sku IS NOT NULL`;
...
module.exports = { sql, ensureTables, parseBody, normalizeModule, MODULES };
