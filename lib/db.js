...
async function logActivity({ action, summary, details, module }) {
  try {
    await sql`
      INSERT INTO activity_log (action, summary, details, module)
      VALUES (${action}, ${summary}, ${details ? JSON.stringify(details) : null}::jsonb, ${module || null})
    `;
  } catch (err) {
    // Never let logging failures break the actual user action
